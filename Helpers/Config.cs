...
namespace DotnetChatbot.Helpers
{
    public sealed class Config
    {
        ...
        {
            s_instance = new Config(configRoot);
        }
        ...
        private static T LoadSection<T>([CallerMemberName] string? caller = null)
        {
            if (s_instance is null)
            {
                throw new InvalidOperationException(
                    "config must be initialized with a call to Initialize(IConfigurationRoot) before accessing configuration values.");
            }
            ...
            return s_instance._configRoot.GetSection(caller).Get<T>() ??
                   throw new Exception($"section: {caller}");
        }
